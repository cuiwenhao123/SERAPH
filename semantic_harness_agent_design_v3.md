...
> 面向 Rust 库的语义感知 Fuzz Harness 自动合成系统设计方案（v2 — 融合 Skills 架构 + CISAL 模式）
...
**关键特征**：Agent 的输出不是最终产物——它只是**中间产物**，由外部领域工具来验证（cargo check）、评估（s3-coverage）和决定是否继续。
...
4. 输出验证外部化
   Agent 的输出由外部工具验证（cargo check、s3-coverage），
   而非 Agent 自我评估。
   这消除了 LLM 的"自我确认偏差"。
...
|---------|-----------|------------|----------|
| **Rust Fuzz Harness (SERAPH)** | s3-extract / s3-model / s3-context | scenario-generator, harness-codegen | cargo check + fuzz |
